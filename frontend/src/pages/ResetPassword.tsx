...
export default function ResetPassword() {
  ...
  const handleReset = async () => {
    if (!allPassed) return toast('Password does not meet requirements', 'error');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, color: 'var(--color-text-primary)' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        style={{ width: '100%', maxWidth: 420, padding: 32, borderRadius: 16, background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border-subtle)' }}>
        ...
        {success ? (
          ...
          <>
            ...
            <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 24 }}>Create a secure password for your account.</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type={showConfirm ? 'text' : 'password'} value={confirm} onChange={(e) => setConfirm(e.target.value)}
                  placeholder="Confirm password" style={{ ...inputStyle, paddingRight: 42 }}
            ...
            <button onClick={handleReset} disabled={loading || !allPassed || !confirm || password !== confirm}
              style={{
                width: '100%', marginTop: 20, padding: 13, borderRadius: 10, border: 'none',
                background: 'var(--color-brand)', color: '#fff',
                fontSize: 14, fontWeight: 600, cursor: loading ? 'wait' : 'pointer',
                opacity: (loading || !allPassed || !confirm || password !== confirm) ? 0.5 : 1,
              }}>
