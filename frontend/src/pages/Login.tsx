...
import { ArrowLeft, Eye, EyeOff, Mail, Lock, Sparkles, ShieldCheck, Zap, Shield } from 'lucide-react';
...
export default function Login() {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      toast('If this email is registered, a reset link has been sent.', 'success');
    } catch { toast('Failed to send reset link', 'error'); }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', display: 'flex' }}>
      ...
      <div className="login-brand-panel" style={{
        ...
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, fontFamily: 'var(--font-display)', color: 'var(--color-text-primary)', marginBottom: 8, lineHeight: 1.2 }}>
            Welcome back
          </h1>
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
            Sign in to manage your institution's cards, memories, and student engagement.
          </p>
        </div>
      ...
      <div className="login-form-panel" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <div style={{
            ...
            background: 'var(--color-bg-tertiary)', marginBottom: 28,
            border: '1px solid var(--color-border-subtle)',
          }}>
            {(['admin', 'student'] as LoginMode[]).map((m) => (
              <button key={m} onClick={() => { setMode(m); setMagicSent(false); setPassword(''); }}
                style={{
          ...
          <AnimatePresence mode="wait">
            {magicSent ? (
              <motion.div key="sent" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <Mail size={40} style={{ color: 'var(--color-accent-green)', marginBottom: 16 }} />
                <p style={{ color: 'var(--color-text-primary)', fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Check your inbox</p>
                <p style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 20 }}>We sent a magic link to {email}</p>
                <button onClick={() => setMagicSent(false)}
                  style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid var(--color-border-default)', background: 'transparent', color: 'var(--color-text-secondary)', cursor: 'pointer', fontSize: 13 }}>
                  Try again
                </button>
              </motion.div>
            ) : (
              <motion.div key={mode} initial={{ opacity: 0, x: mode === 'admin' ? -8 : 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {/* Email */}
                  <div style={{ position: 'relative' }}>
                    <Mail size={16} style={iconStyle} />
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                      placeholder={mode === 'admin' ? 'admin@college.edu' : 'your.email@college.edu'}
                      style={inputStyle}
                      onKeyUp={(e) => e.key === 'Enter' && (mode === 'admin' ? handleAdminLogin() : handleStudentLogin())} />
                  </div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)}
                      placeholder={mode === 'admin' ? 'Password' : 'Password (optional — blank for magic link)'}
                ...
                {mode === 'student' && password && (
                  <div style={{ fontSize: 11, color: 'var(--color-text-muted)', lineHeight: 1.6, padding: '0 4px' }}>
                    <p style={{ marginBottom: 4, fontWeight: 600, fontSize: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>Password must have:</p>
                    {[
                      ...
                      <div key={r.text} style={{ display: 'flex', alignItems: 'center', gap: 6, color: r.ok ? 'var(--color-accent-green, #22C55E)' : 'var(--color-text-muted)' }}>
                        <span style={{ fontSize: 10 }}>{r.ok ? '✓' : '○'}</span> {r.text}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {/* Submit */}
                <button onClick={mode === 'admin' ? handleAdminLogin : handleStudentLogin}
                  disabled={loading || !email || (mode === 'admin' && !password)}
                  style={{
                    width: '100%', marginTop: 20, padding: 13, borderRadius: 10, border: 'none',
                    background: 'var(--color-brand)', color: '#fff',
                    fontSize: 14, fontWeight: 600, cursor: loading ? 'wait' : 'pointer',
                    opacity: (loading || !email || (mode === 'admin' && !password)) ? 0.5 : 1,
                    transition: 'opacity 0.2s',
                  }}>
                  {loading ? <LoadingSpinner size="sm" /> : (
                    mode === 'admin' ? 'Sign In' : (password ? 'Sign In' : 'Send Magic Link')
                  )}
                </button>
                ...
                <div style={{ textAlign: 'right', marginTop: 8 }}>
                  <button onClick={handleForgotPassword} disabled={forgotLoading}
                    style={{ background: 'none', border: 'none', color: 'var(--color-brand)', cursor: 'pointer', fontSize: 12, fontWeight: 500, opacity: forgotLoading ? 0.5 : 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '20px 0' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: 1 }}>or</span>
                  <div style={{ flex: 1, height: 1, background: 'var(--color-border-subtle)' }} />
                </div>
                ...
                <button onClick={handleDemoLogin}
                  ...
                  <Sparkles size={14} /> Demo {mode === 'admin' ? 'Admin' : 'Student'} Portal
                </button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <p style={{ textAlign: 'center', marginTop: 24, fontSize: 13, color: 'var(--color-text-muted)' }}>
            ...
            <span onClick={() => navigate('/register')} style={{ color: 'var(--color-brand)', cursor: 'pointer', fontWeight: 500 }}>
