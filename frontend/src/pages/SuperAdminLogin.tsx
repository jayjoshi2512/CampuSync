...
export default function SuperAdminLogin() {
  ...
  // DEMO mode
  const isDemo = useAuthStore((s) => s.isAuthenticated && s.role === 'super_admin');
  ...
  return (
    <div style={{
      ...
      <GlassCard elevation={2} glow style={{ maxWidth: 420, width: '100%', padding: 40 }}>
        <div style={{
          width: 48, height: 48, borderRadius: 12,
          background: 'linear-gradient(135deg, var(--color-accent-green), var(--color-brand))',
        ...
        <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 28 }}>
          Restricted access. Click below to receive a secure access code at your registered email.
        </p>
          ...
          {step === 'otp' && (
            <motion.div key="otp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 12 }}>
                Access code sent to <strong style={{ color: 'var(--color-accent-green)' }}>{maskedEmail}</strong>
              </p>
              <input
              ...
              <button onClick={() => { setStep('init'); setOtp(''); }}
                style={{
